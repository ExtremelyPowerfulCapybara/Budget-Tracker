(function(){
  ...
  function renderCategoryBarsMarkup(options){
    ...
    return categories.map(category=>{
      ...
      return '<div class="category-bar"><div class="category-bar-header"><div class="category-bar-name" style="color:'+esc(category.color)+'">'+esc(category.label)+'</div><div class="category-bar-nums"><span>'+formatMoney(actual)+'</span>'+(goal?' / '+formatMoney(goal):' <span class="bar-no-goal">Sin meta</span>')+'</div></div>'+(goal?'<div class="bar-track'+(over?' over':'')+'"><div class="bar-fill" style="width:'+pct+'%;background:'+esc(category.color)+'"></div></div><div class="bar-pct'+(over?' over':'')+'">'+pct.toFixed(0)+'%'+(over?' \u00b7 excedido':'')+'</div>':'<div class="bar-pct bar-pct-empty">Configura una meta para ver el avance</div>')+'</div>';
    }).join('');
  }
