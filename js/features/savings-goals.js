(function(){
  ...
  function renderSavingsGoalsMarkup(options){
    ...
    const cards=savingsGoals.map(goal=>{
      ...
      const done=pct>=100;
      return `<div class="savings-card"><div class="savings-card-header"><div class="savings-card-name" style="color:${esc(goal.color)}">${esc(goal.name)}</div><div><div class="savings-card-amounts"><span>${formatMoney(saved)}</span> / ${formatMoney(goal.target)}</div><div class="savings-card-pct${done?' done':''}">${pct.toFixed(0)}%${done?' \u00b7 Completada':''}</div></div></div><div class="bar-track"><div class="bar-fill" style="width:${pct}%;background:${esc(goal.color)}"></div></div><div class="savings-card-meta">Ahorro asignado a esta meta: ${formatMoney(saved)}</div><div style="display:flex;gap:8px;margin-top:10px;"><button class="entry-btn" style="flex:1;width:auto;border-radius:8px;" data-sg-action="edit" data-sg-id="${goal.id}">&#9998; Editar</button><button class="entry-btn delete" style="width:auto;padding:0 12px;border-radius:8px;" data-sg-action="delete" data-sg-id="${goal.id}">&#10005;</button></div></div>`;
    }).join('');
